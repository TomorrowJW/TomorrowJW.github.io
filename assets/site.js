(function () {
  ...
  themeButton.innerHTML = '<span data-lang="zh">Q版风格</span><span data-lang="en">Q Style</span>';
  if (languageSwitch) languageSwitch.insertBefore(themeButton, languageSwitch.firstChild);

  const qThemeMediaRules = [];
  Array.from(document.styleSheets).forEach(function (styleSheet) {
    ...
    const markerIndex = rules.findIndex(function (rule) {
      return rule.type === CSSRule.MEDIA_RULE && rule.cssText.indexOf("--paper-shadow: 5px 6px 0") !== -1;
    });
    if (markerIndex === -1) return;

    rules.slice(markerIndex).forEach(function (rule) {
      if (rule.type === CSSRule.MEDIA_RULE) {
        qThemeMediaRules.push({ rule: rule, mediaText: rule.media.mediaText });
      }
    });
  });

  function setQTheme(active) {
    const enabled = Boolean(active && qThemeMediaRules.length);
    qThemeMediaRules.forEach(function (item) {
      item.rule.media.mediaText = enabled ? item.mediaText : "not all";
    });
    body.classList.toggle("theme-q", enabled);
    document.documentElement.classList.toggle("theme-q", enabled);
    themeButton.classList.toggle("is-active", enabled);
    themeButton.setAttribute("aria-pressed", String(enabled));

    const themeColor = document.querySelector('meta[name="theme-color"]');
    if (themeColor) themeColor.setAttribute("content", enabled ? "#f5ecd9" : "#0b2f53");

    const favicon = document.querySelector('link[rel="icon"]');
    if (favicon) {
      favicon.setAttribute("href", enabled ? "./images/jw-chibi-mascot.png" : "./images/logo-ahnu.jpeg");
      favicon.setAttribute("type", enabled ? "image/png" : "image/jpeg");
    }

    try {
      localStorage.setItem("jw-theme-q-default-v2", String(enabled));
    } catch (error) {
      // Theme switching still works when storage is unavailable.
    }
  }

  let initialQTheme = true;
  try {
    initialQTheme = localStorage.getItem("jw-theme-q-default-v2") !== "false";
  } catch (error) {
    initialQTheme = true;
  }
  setQTheme(initialQTheme);

  themeButton.addEventListener("click", function () {
    setQTheme(!body.classList.contains("theme-q"));
  });

  const brand = document.querySelector(".brand");
  if (brand && !brand.querySelector(".brand-mascot")) {
    const mascot = document.createElement("img");
    mascot.className = "brand-mascot";
    mascot.src = "./images/jw-chibi-mascot.png";
    mascot.alt = "";
  ...
  if (portraitWrap && !portraitWrap.querySelector(".hero-mascot")) {
    const mascot = document.createElement("img");
    mascot.className = "hero-mascot";
    mascot.src = "./images/jw-chibi-mascot.png";
    mascot.alt = "";
    mascot.setAttribute("aria-hidden", "true");
    portraitWrap.appendChild(mascot);
  }
